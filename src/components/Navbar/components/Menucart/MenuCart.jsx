...
function MenuCart({ showCart, hiddenCart }) {
    ...
    return (
        <section className={`menu-navbar right-0 h-[91.2vh] border-l border-black ${isHidden}`}>
            ...
            <p className='my-2 text-2xl'> {`Total: Rs. ${totalPrice}`} </p>
            <button
                aria-label='go to the cart page'
                className='text-pink-400 px-4 py-1 mt-2 mb-4 lg:text-2xl border border-black rounded-md focus:ring-4 transform active:scale-75 transition-transform font-semibold text-2xl hover:bg-gray-200'
                onClick={hiddenCart}
            >
